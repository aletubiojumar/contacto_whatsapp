"""Genera notas JSONL de AMA a partir de los reportes de Peritoline."""
...
def _build_text(visita: str) -> str:
    ...
    if visita:
        return f"Intervención concertada para la fecha {visita}."
    return (
        "Se ha contactado con el Mutualista a la espera de concretar fecha intervención."
    )
...
def main() -> None:
    """Ejecuta la generacion de JSONL de AMA desde los reportes.
    ...
    setup_logging()
    logger = get_logger(tarea="peritoline_ama")
    parser = argparse.ArgumentParser(
        description="Genera data/notas_ama.jsonl comparando reportes de Peritoline."
    )
    parser.add_argument(
        "--latest",
        default="data/peritoline/raw_ama/ama_report_latest.xlsx",
        help="Ruta del ultimo reporte descargado.",
    )
    parser.add_argument(
        "--previous",
        default="data/peritoline/raw_ama/ama_report_previous.xlsx",
    ...
    logger.info("Notas AMA generadas: %s (%s entradas)", output_path, len(notas))
