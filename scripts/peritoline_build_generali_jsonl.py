"""Genera notas JSONL de Generali a partir de los reportes de Peritoline."""
...
def main() -> None:
    """Ejecuta la generacion de JSONL de Generali desde los reportes.
    ...
    setup_logging()
    logger = get_logger(tarea="peritoline_generali")
    parser = argparse.ArgumentParser(
        description="Genera data/notas_generali.jsonl comparando reportes de Peritoline."
    )
    parser.add_argument(
        "--latest",
        default="data/peritoline/raw_generali/generali_report_latest.xlsx",
        help="Ruta del ultimo reporte descargado.",
    )
    parser.add_argument(
        "--previous",
        default="data/peritoline/raw_generali/generali_report_previous.xlsx",
    ...
    logger.info("Notas Generali generadas: %s (%s entradas)", output_path, len(notas))
