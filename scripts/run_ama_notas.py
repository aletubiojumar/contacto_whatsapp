"""Runner para ejecutar notas de AMA desde JSONL y Peritoline."""
...
def _load_notas_ama(path: Path) -> List[Dict[str, str]]:
    """Lee el archivo JSONL de AMA.
...
def main() -> None:
    """Ejecuta el runner de notas AMA.
    ...
    parser = argparse.ArgumentParser(
        description="Ejecuta notas de AMA leyendo un JSONL."
    )
    parser.add_argument(
        "--notas-jsonl",
        default="data/notas_ama.jsonl",
        help="Ruta al JSONL de AMA.",
    )
    args = parser.parse_args()

    notas = _load_notas_ama(Path(args.notas_jsonl))
    logger.info("Iniciando notas AMA (%s entradas)", len(notas))
    config = load_config()
    config.keep_browser_open = True
    config.keep_browser_open_flag = True
    ...
    with launch_browser(config) as (_, page):
        ...
        logger.info("Edicion de AMA abierta")
        ...
        logger.info("Portal AMA abierto desde Claves")
        run_ama_with_page(ama_page, config, notas)

    logger.info("Notas AMA finalizadas")
